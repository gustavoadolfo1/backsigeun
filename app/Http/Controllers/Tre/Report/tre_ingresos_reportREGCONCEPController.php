...
$_param = ""; $_au = ""; $_aco = "";
class tre_ingresos_reportREGCONCEPController extends Controller{ 
    public function report(Request $data){
        ...
        $_rec = app('App\Http\Controllers\Tre\grl_conceptosController')->grl_conceptos_select($data,Array("ConcepId"=>$data->get("ConcepId"),"TypeRecord"=>"win","RecordLimit"=>1));
        ...
        if ( $data->get("pType") == 33 ) {
            ...
            $_sheet1->setCellValue("A1", "REGISTRO DE INGRESOS SEGUN CONCEPTO DE GESTION");
        ...
        $pdf->setFile_header("tre_ingresos_report_regconcep_head.php");
        ...
        foreach ($_ac as $row) { $_nro++;
            ...
            //if ( $row["iIngId"] != $_id ) { $_nro++; $_impt += $row["nIngImpt"]; $_b = "T";
                ...
                $_ConcepReqNombre = utf8_encode(substr(utf8_decode($row["cConcepReqNombrex"]),0,40));
                $pdf->Cell(65, $pdf->h_row, $_ConcepReqNombre, "T", 0, "L");
            ...
            //$pdf->Cell(50, $pdf->h_row, $_ConcepReqNombre, "T", 0, "L");
	    ...
	    $pdf->Cell(256, $pdf->h_row, "TOTAL  IMPORTE  ", "TR", 0, "R");
