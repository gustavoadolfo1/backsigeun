...
$_param = ""; $_au = ""; $_aedg = "";
class tre_ingresos_reportRESESPEDETController extends Controller{ 
    public function report(Request $data){
        ...
        $_t = (count($_aed) <= 7 ? "P" : "L");
        $pdf = new PDF($_t, "mm", "A4", true, "UTF-8", false);
        $pdf->setFile_header("tre_ingresos_report_resespedet_head.php");
        $pdf->SetFillColor(192, 192, 192);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->h_row = 5;  $pdf->max = 265;  $_nro = 0;   $_impt = 0;  $_id = 0;  $_item = 0;

        $pdf->fnNewPage(2500);  $pdf->SetFont("helvetica", "", 6);
        //$pdf->SetLineStyle(array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 3));
        $_coltot = (count($GLOBALS["_aedg"]) <= 7 ? 7 : 14); $espedet = array();
        for ($i = 1; $i <= $_coltot; $i++) {
            $espedet[$i] = 0;
        }
        ...
        foreach ($_ac as $row) { $_nro++;
            ...
            $pdf->Cell(20, $pdf->h_row, $row["cIngDocument"], 1, 0, "L");
            $pdf->Cell(16, $pdf->h_row, $GLOBALS["_fn"]->fnDateDDMMAAAA($row["dDocFecha"]), 1, 0, "C");
            $pdf->Cell(18, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($row["nIngImpt"],2), 1, 0, "R");
            $nroespe = 0;
            foreach ($_aed as $r) { $nroespe++;
                $pdf->Cell(17, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($row[$r["cEspeDetCodigo"]],2), 1, 0, "R");
                $espedet[$nroespe] = $espedet[$nroespe]*1 + $row[$r["cEspeDetCodigo"]]*1;
            }
            for ($i = 1; $i <= $_col; $i++) {
                $pdf->Cell(17, $pdf->h_row, "", 1, 0, "C");
            }
	    ...
	    $pdf->Cell(44, $pdf->h_row, "TOTAL  IMPORTE  ", "TR", 0, "R");
	    $pdf->Cell(18, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($_impt*1), 1, 0, "R", 1);
        for ($i = 1; $i <= $_coltot; $i++) {
            $pdf->Cell(17, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($espedet[$i]*1), 1, 0, "R", 1);
        }
