...
$_param = ""; $_fn = ""; $_au = "";
class grl_conceptos_requisitos_reportREGController extends Controller{ 
    public function report(Request $data){
        ...
        if ( $data->get("pType") == 33 ) {
            ...
            $_sheet1->getColumnDimension("R")->setWidth(16);
            $_sheet1->getColumnDimension("S")->setWidth(60);
            $_sheet1->getColumnDimension("T")->setWidth(60);
            $_cf = "T";
    
            $_sheet1->setCellValue("A1", "REGISTRO CONCEPTOS DE INGRESO");
            ...
            $_sheet1->setCellValueByColumnAndRow(18, $_fila, "Clasificador");
            $_sheet1->setCellValueByColumnAndRow(19, $_fila, "Descripción Clasificador");
            $_sheet1->setCellValueByColumnAndRow(20, $_fila, "Enlace Académico");
            ...
            foreach ($_ac as $row) { $_fila++; $_nro++;
                ...
                $_sheet1->setCellValue("R".$_fila, $row["cEspeDetCodigo"]);
                $_sheet1->setCellValue("S".$_fila, $row["cEspeDetNombre"]);
                $_sheet1->setCellValue("T".$_fila, $row["cConcepEnlacNombre"]);
        ...
        $pdf->setFile_header("grl_conceptos_requisitos_report_reg_head.php");
        $pdf->SetFillColor(192, 192, 192);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->h_row = 4;  $pdf->max = 182;  $_nro = 0;

        $pdf->fnNewPage(2500);  $pdf->SetFont("helvetica", "", 6);
        $pdf->SetLineStyle(array('width' => 0.3, 'cap' => 'butt', 'join' => 'miter', 'dash' => 4));
        foreach ($_ac as $row) {
    		...
    		$pdf->Cell(8, $pdf->h_row, $_nro, "", 0, "R");
            $pdf->Cell(80, $pdf->h_row, $row["cConcepReqNombrex"], "", 0, "L");
            ...
            $pdf->Cell(8, $pdf->h_row, $row["iConcepReqOnlyExter"] == 1 ? "A" : "", "", 0, "C");
            ...
            $pdf->Cell(118, $pdf->h_row, substr($row["_DepenNombre"],0,80), "B", 0, "L");
        ...
        return $pdf->Output('registro conceptos.pdf', 'S');
    }
}