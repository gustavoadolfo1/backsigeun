...
$_param = ""; $_fn = ""; $_au = "";
class tre_ingresos_reportCONESPEDETController extends Controller{ 
    public function report(Request $data){
        ...
        if ( $data->get("pType") == 33 ) {
            ...
            $_xls->getProperties()->setCreator("Sigeun")->setTitle("Consolidado x Clasificador");
            ...
            $_sheet1->setTitle("Consolidado x Clasificador");
            $_sheet1->getDefaultRowDimension()->setRowHeight(15);
    
            $_sheet1->getColumnDimension("A")->setWidth(9);
            $_sheet1->getColumnDimension("B")->setWidth(16);
            $_sheet1->getColumnDimension("C")->setWidth(80);
            $_sheet1->getColumnDimension("D")->setWidth(8);
            $_sheet1->getColumnDimension("E")->setWidth(14);
            $_cf = "E";
    
            $_sheet1->setCellValue("A1", "CONSOLIDADO DE INGRESOS CAJA x CLASIFICADOR");
            ...
            $_sheet1->setCellValueByColumnAndRow(2, $_fila, "Clasificador");
            ...
            foreach ($_ac as $row) { $_fila++; $_nro++;
                ...
                $_sheet1->setCellValue("B".$_fila, $row["cEspeDetCodigo"]);
                $_sheet1->setCellValue("C".$_fila, $row["cEspeDetNombre"]);
                $_sheet1->setCellValue("D".$_fila, $row["iNumReg"]);
                $_sheet1->setCellValue("E".$_fila, $row["nEspeDetImpt"]);
            }
            ...
            $_sheet1->getStyle("D".$_fila.":D".$_fila)->getAlignment()->applyFromArray(array('horizontal' => \PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_RIGHT));
            ...
            header('Content-Disposition: attachment; filename="Consolidado Ingresos Caja x Clasificador.xlsx"');
        ...
        $pdf->setFile_header("tre_ingresos_report_conespedet_head.php");
        ...
        foreach ($_ac as $row) { 
            ...
            $pdf->Cell(127, $pdf->h_row, $row["cEspeDetNombre"], 1, 0, "L");
		    $pdf->Cell(10, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($row["iNumReg"], 0), 1, 0, "R");
            $pdf->Cell(20, $pdf->h_row, $GLOBALS["_fn"]->fnNumFormat($row["nEspeDetImpt"]), 1, 0, "R");
