...
$_param = ""; $_fn = ""; $_au = ""; 
class grl_conceptos_requisitos_reportREGESPEDETController extends Controller{ 
    public function report(Request $data){
        ...
        if ( $data->get("pType") == 33 ) {
            ...
            $_sheet1->getColumnDimension("Q")->setWidth(60);
            $_sheet1->getColumnDimension("R")->setWidth(60);
            $_cf = "R";
    
            $_sheet1->setCellValue("A1", "REGISTRO CONCEPTOS DE INGRESO x CLASIFICADOR PRESUPUESTAL");
            ...
            $_sheet1->setCellValueByColumnAndRow(17, $_fila, "Unidad Orgánica");
            $_sheet1->setCellValueByColumnAndRow(18, $_fila, "Enlace Académico");
            ...
            $_nro = 0; $_filaIni = ($_fila+1); $_EspeDetCodigo = "";
            foreach ($_ac as $row) { 
                if ( $_EspeDetCodigo != $row["cEspeDetCodigo"] ){
                    $_fila++;
                    $_sheet1->getStyle("A".$_fila.":S".$_fila)->getFont()->setBold(true);
                    $_sheet1->setCellValue("B".$_fila, $row["cEspeDetCodigo"]==""?"###################":$row["cEspeDetCodigo"]."  -  ".$row["cEspeDetNombre"]);
                }
                ...
                $_sheet1->setCellValue("Q".$_fila, $row["_DepenNombre"]);
                $_sheet1->setCellValue("R".$_fila, $row["cConcepEnlacNombre"]);
                $_sheet1->getStyle("A".$_fila.":R".$_fila)->getFont()->getColor()->setARGB($row["iConcepReqEstado"]*1 > 0 ? Color::COLOR_BLACK : Color::COLOR_RED);

                $_EspeDetCodigo = $row["cEspeDetCodigo"];
            }
        ...
        $pdf->setFile_header("grl_conceptos_requisitos_report_regespedet_head.php");
        $pdf->SetFillColor(192, 192, 192);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->h_row = 4;  $pdf->max = 185;  $_nro = 0;  $_item = 0;  $_espedet = "=";

        $pdf->fnNewPage(2500);
        foreach ($_ac as $row) { $_nro++;
            if ( $row["cEspeDetCodigo"] != $_espedet ) { $_item = 0;
                ...
                $pdf->Cell(149, $pdf->h_row, $row["cEspeDetCodigo"]==""?str_repeat ("#",40):$row["cEspeDetCodigo"]." - ".$row["cEspeDetNombre"], "B", 0, "L");
            }
            $_item++;
            ...
            $pdf->Cell(118, $pdf->h_row, $row["_DepenNombre"], "B", 0, "L");
            $pdf->Cell(18, $pdf->h_row, "", "B", 0, "L");
            $pdf->Cell(81, $pdf->h_row, "", "B", 0, "L");
            $pdf->Cell(47, $pdf->h_row, substr($row["cConcepEnlacNombre"],0,40), "B", 0, "L");

            $_espedet = $row["cEspeDetCodigo"];
        }
