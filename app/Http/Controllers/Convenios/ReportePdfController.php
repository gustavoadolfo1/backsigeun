...

{
    ...
    public function funcionconvenio( $idAnyo){
        $dataEnviar =[$idAnyo];
        $datafecha = \DB::select('EXEC con.Sp_SEL_yearsXiYearId ?', $dataEnviar);
        $resumen = \DB::select('EXEC con.[Sp_CONVENIOS_SEL_Convenios_Muestra_ConveniosXAnho] ?', array( $idAnyo));

        $pdf = \PDF::loadView('convenios.blade', compact(['resumen', 'datafecha']))->setPaper('A4');
        return $pdf->download("cualquiernombre.pdf");
    }
