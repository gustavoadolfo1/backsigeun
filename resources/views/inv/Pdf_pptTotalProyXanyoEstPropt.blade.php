...
@if (count($data2)!==0)
    @foreach($data2 as $index=>$a)
        @if ($a->anyo <> $anyoAnt)
            ...
            @if ($anyoAnt <> '')
                    <tr style="font-size: 10px; text-align: center">
                        <td colspan="3" style="text-align:right; font-size: 11px;"><strong>Total</strong></td>
                        <td style="text-align: right;">{{ $totalNroProy }}</td>
                        <td style="text-align: right;">{{ number_format(($totalAprobado), 2, '.', '') }}</td>
                        <td style="text-align: right;">{{ number_format(($totalAsignado), 2, '.', '') }}</td>
                        <td style="text-align: right;">{{ number_format(($totalEjecutado), 2, '.', '') }}</td>
            ...
            @if ($a->iEstadoPropuesta <> $estadoPropuestaAnt)
                @switch($a->iEstadoPropuesta)
                    @case(0)
                    <h3 style="text-align:center"><strong>Presupuesto Total de Proyectos por Año</strong></h3>
                    @break

                    @case(1)
                    <h3 style="text-align:center"><strong>Presupuesto Total de Propuestas de Proyectos por Año</strong></h3>
                    @break
                @endswitch
            ...
            <table width="100%" style="border: 1px solid #2B2E4A" cellspacing="0" cellpadding="3" border="1">
                ...
                <tr class="card-header" style="font-size:11px;background: #2B2E4A; color:white">
                    ...
                    <th width="34%" style="text-align:center"><strong>Tipo de proyecto</strong></th>
                    <th width="20%" style="text-align:center"><strong>Resolución que aprueba</strong></th>
                    <th width="5%" style="text-align:center"><strong>Cantidad</strong></th>
                    <th width="10%" style="text-align:center"><strong>Aprobado</strong></th>
                    <th width="10%" style="text-align:center"><strong>Asignado</strong></th>
                    <th width="10%" style="text-align:center"><strong>Ejecutado</strong></th>
                    <th width="10%" style="text-align:center"><strong>Pendiente</strong></th>
                </tr>
                </thead>
                <tbody>
                <tr style="font-size: 10px">
                    ...
                    <td style="text-align: right;">{{ $a->nroProyectos }}</td>
                    <td style="text-align: right;">{{ number_format(($a->presupuestoAprobado), 2, '.', '') }}</td>
                    <td style="text-align: right;">{{ number_format(($a->presupuestoAsignado), 2, '.', '') }}</td>
                    <td style="text-align: right;">{{ number_format(($a->ejecutado), 2, '.', '') }}</td>
                    <td style="text-align: right;">{{ number_format(($a->pendiente), 2, '.', '') }}</td>
                </tr>
        @php
            $anyoAnt = $a->anyo;
        ...
        @php
            ...
            $totalAprobado += $a->presupuestoAprobado;
            $totalAsignado += $a->presupuestoAsignado;
            $totalEjecutado += $a->ejecutado;
            $totalPendiente += $a->pendiente;
        @endphp
    @endforeach
                <tr style="font-size: 10px; text-align: center">
                    <td colspan="3" style="text-align:right; font-size: 11px;"><strong>Total</strong></td>
