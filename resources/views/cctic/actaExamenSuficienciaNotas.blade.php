...
<body>
    <div class="page-container">
        ...
        <h5 class="text-uppercase text-center mt-4">ACTA DE EXAMEN DE SUFICIENCIA</h5>
        <div class="page-content">
            <p>
                ...
                para la aplicación del examén de SUFICIENCIA del curso de
            ...
            <table class="table-custom table table-sm table-bordered w-50">
                <tbody>
                    ...
                    <tr>
                        <td style="font-size: 12px;">{{ $item->cTipoCriterio }}</td>
                        <td style="font-size: 12px;">{{ $item->nNota }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <p>Obteniendo como promedio: <span class="text-uppercase"> NOTA PROMEDIO = {{ $data->Promedio_Acta }}</span></p>
            <p>Con lo cual se concluye que el aspirante <span class="text-uppercase">{{ $data->cPersNombre.' '.$data->cPersPaterno }} {{ $data-> cPersMaterno }},</span>
                ha {{ $data->estado }} el examen de suficiencia de {{ $data->cPerfilProfesional }} con la nota promedio {{ $data->Promedio_Acta }}</p>
